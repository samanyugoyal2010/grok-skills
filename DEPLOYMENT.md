...
The MCP endpoint is `/mcp`. A non-loopback bind requires both `MCP_HTTP_AUTH_TOKEN` and `MCP_HTTP_ALLOWED_HOSTS`; do not expose the server publicly without a token and an appropriate `MCP_HTTP_ALLOWED_ORIGINS` value. Requests are capped at 256,000 bytes by default. List allowed hostnames without ports.
